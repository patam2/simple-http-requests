...
class Session:
    def __init__(self, url: str, port = None) -> None:
        self.host, self.page, self.port = formatting.parse_url(url)
        if port:
            self.port = port #todo find a better way for this
    ...
    def receive(self):
        r = self.socket_session.recv(BLOCKSIZE)
        if b'\r\n\r\n' in r:
            data, received = r.split(b"\r\n\r\n", 1)
        elif b'\r\n' in r:
            data, received = r.split(b"\r\n", 1)
    ...
    def request(self, method: str, page: str, headers={}, data=None, **kwargs) -> Response:
        ...
        data, received = self.receive()
        if b'HTTP/1.0' in data:
        ...
        if "content-length" not in headers and received:
            while True:
